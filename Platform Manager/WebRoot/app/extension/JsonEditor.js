Ext.define('Asc.extension.JsonEditor', {
	...
	createPicker : function() {
		...
		var tree =	Ext.create('Ext.tree.Panel', {
			 ...
			 * 增加新节点
			 */
			appendAttributeNode : function(parentNode, name, val, type){
  				var editable = true;
	        ...
	        initTree : function(obj, node){
	        	...
	        	for(var i = 0; i < obj.length; i++){
	        		var childnode = me.tree.appendAttributeNode(node,obj[i].text,obj[i].value,obj[i].type);
			...
			createNode : function(name, type, initialValue){
				for(var n in initialValue){
					if(initialValue[n]['name']==name && initialValue[n]['type']==type){
						var childnode = me.tree.appendAttributeNode(me.tree.getRootNode(),initialValue[n]['name'],initialValue[n]['value'],initialValue[n]['type']);
			...
			*/
			createTrunk : function(node,children){
				for(var i = 0; i < children.length; i++){
	        		var childnode = me.tree.appendAttributeNode(node,children[i]['name'],children[i]['value'],children[i]['type']);
